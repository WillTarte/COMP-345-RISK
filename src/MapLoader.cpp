...
bool
MapLoader::validateCountryLine(int *countryCount, std::vector<std::string> *lineWords, const int *lineCount, bool *validMap,
    ...
    if (lineWords->size() < 3) {
        ...
        *validMap = false;
        ...
        if(lineWords->size() > 3) std::cout << "Line " << *lineCount << " - [WARNING] : a line in  the countries declaration had extra tokens.\n";
