...
/**
 * Copy constructor for Player class. Does a deep copy
 *
 * @param other the Player object to copy
 */
Player::Player(const Player& other){
    this->pOwnedCountries = std::copy(other.pOwnedCountries);
...
int Player::attack() {
    ...
    if (playerChoice == 'y') {
        do {
            ...
            std::cout << "\nYou own " << this->getOwnedCountries()->size()
                      << " countries. From which country do you wa->t to attack?(choose 0 to "
    ...
    return this->executeAttack(*fromCountry, *toCountry, (Player&) nullptr, numAttackingDice, numDefendingDice);
}